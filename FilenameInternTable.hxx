...
public:
  ...
  const id_type id(const std::string& fname) const { return id_lookup_.at(fname); }

  const std::string& name(const id_type& id) const { return table_[id]; }

  const std::vector<std::string>::size_type size() const { return table_.size(); }
