...
public:
  ...
  const FilenameInternTable& get_filename_table() const { return ftable_; }
  ...
  void populate_dependency_map(dependency_map_type& depmap) const {
    for (auto const& r : records_)
      for (auto const& ofid : r.second->outfile_ids)
        for (auto const& ifid : r.second->infile_ids) depmap.insert(std::make_pair(ofid, ifid));
  }
...
private:
  struct ProcIORecord {
    ...
    ProcIORecord(const pid_t& procid) : pid(procid) {}
  };
